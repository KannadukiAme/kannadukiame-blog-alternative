...
export default function Article({ id, title, date, imageUrl, description, tags }) {
  return (
    <div className="flex bg-white my-5">
      ...
      <div className="md:w-2/3 px-8">
        <div className="mb-3"><Link className="text-3xl font-bold hover:text-sora" href={`/posts/${id}`}>{title}</Link></div>
        <div className="mb-2 text-gray-400 text-lg"><FontAwesomeIcon className="mr-2" icon={faCalendarAlt} />{date}</div>
